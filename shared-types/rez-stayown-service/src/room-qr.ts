 ...
 */
export async function recordServiceCharge(charge: Omit<ServiceCharge, 'id' | 'synced' | 'createdAt' | 'updatedAt'>): Promise<ServiceChargeDocument> {
  try {
...
/**
 * Sync a charge to StayOwn folio
 */
export async function syncChargeToFolio(charge: ServiceChargeDocument): Promise<void> {
  ...
  try {
    // Call StayOwn bridge to add charge to folio
    // This would integrate with the booking/payment system
    const bridgeUrl = process.env.STAYOWN_BRIDGE_URL || 'http://localhost:4015';

    await axios.post(`${bridgeUrl}/api/folio/charge`, {
      bookingId: charge.bookingId,
      hotelId: charge.hotelId,
      category: charge.category,
      description: charge.description,
    ...
    console.log(`[RoomQR] Charge ${charge.id} synced to folio`);
  } catch (error) {
...
export async function handleRoomServiceWebhook(event: RoomServiceWebhookEvent): Promise<void> {
  ...
  switch (event.event) {
    case 'request.created':
      ...
      console.log(`[RoomQR] New ${event.data?.serviceType} request for booking ${event.bookingId}`);
      break;

    case 'request.completed':
      // Record the charge when service is completed
      if (event.data?.totalAmountPaise > 0) {
        await recordServiceCharge({
          bookingId: event.bookingId,
          hotelId: event.hotelId,
          roomId: event.roomId,
          category: event.data.serviceType || 'other',
          description: event.data.description || `${event.data.serviceType} service`,
